...
def whats_new(session):
    ...
    for section in tqdm(sections_by_python):
        ...
        session_new = requests_cache.CachedSession()
        response = get_response(session, version_link)
        if response is None:
            continue
        soup = BeautifulSoup(response.text, features='lxml')
        title_tag = find_tag(soup, 'h1')
        reference_tag = find_tag(soup, 'dl')
        reference_tag_text = reference_tag.text.replace('\n', ' ')
        results.append(
            (version_link, title_tag.text, reference_tag_text)
        )
...
def latest_versions(session):
    ...
    if response is None:
        return
    soup = BeautifulSoup(response.text, features='lxml')
    sidebar = soup.find_tag(soup, 'div', attrs={'class': 'menu-wrapper'})
